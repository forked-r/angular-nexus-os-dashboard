...
import { DashboardService, Communication } from "../../services/dashboard.service"
    ...
    <div class="bg-slate-900/50 border-slate-700/50 backdrop-blur-sm rounded-lg border">
      <div class="pb-2 flex flex-row items-center justify-between px-6 pt-6">
        <div class="text-slate-100 flex items-center text-base font-medium">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" 
            class="mr-2 h-5 w-5 text-blue-500" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
          </svg>
          Communications Log
        </div>
        <span class="bg-slate-800/50 text-blue-400 border border-blue-500/50 text-xs px-2 py-0.5 rounded-full">
          4 New Messages
        </span>
      </div>
      <div class="px-6">
        <div class="space-y-3">
          <div *ngFor="let comm of communications$ | async" 
            class="flex space-x-3 p-2 rounded-md"
            [ngClass]="{'bg-slate-800/50 border border-slate-700/50': comm.unread}">
            <div class="relative inline-block h-8 w-8 rounded-full bg-slate-700 text-cyan-500 flex items-center justify-center overflow-hidden">
              <img [src]="comm.avatar" [alt]="comm.sender" class="h-full w-full object-cover" />
              <span class="absolute inset-0 flex items-center justify-center">{{ comm.sender.charAt(0) }}</span>
            </div>
            <div class="flex-1">
              <div class="flex items-center justify-between">
                <div class="text-sm font-medium text-slate-200">{{ comm.sender }}</div>
      ...
      <div class="border-t border-slate-700/50 pt-4 px-6 pb-6 mt-4">
        <div class="flex items-center w-full space-x-2">
          ...
          />
          <button class="bg-blue-600 hover:bg-blue-700 h-8 w-8 rounded-md flex items-center justify-center border-none">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" 
              class="h-4 w-4 text-white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <button (click)="sendMessage()" class="bg-cyan-600 hover:bg-cyan-700 h-8 w-8 rounded-md flex items-center justify-center border-none">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" 
              class="h-4 w-4 text-white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path>
            </svg>
